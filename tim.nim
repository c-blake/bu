...
proc me(s: RunningStat): (float, float) =
  (s.mean, s.standardDeviation/s.n.float.sqrt)

proc tim(best=3, runs=10, sigma=7.5, write="", cmds: seq[string]) =
  ## Run shell commands (maybe w/escape/quoting) 2*R times.  Finds mean,stderr
  ## of the `best` `runs` twice and, if stable at `sigma`-level, merge results
  ## (reporting mean,stderr of the `best` of all runs).
  ...
  for cmd in cmds:
    ...
    if sigmas > sigma:
      let sa = &"{sigmas:.1f}"
      echo &"UNSTABLE; Mean,StdErr(Best {best} of {runs}):\n\t", 
           fmtUncertain(m1, e1), "\n\t", fmtUncertain(m2, e2), "\n", &"""
...
OS|BIOS) may stabilize timing as can suspend/quit of competing work/browsers."""
    else:       # Compute best of all runs & print mean,stderr
