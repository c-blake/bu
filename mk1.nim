...
proc mk1(file="/dev/stdin", nl='\n', meta='%', explain=false, keep=false,
  ...
  ## already quoted if necessary input-output pairs.  If ages indicate updating,
  ## `%[io]` are interpolated into `cmd`.  `mk1` only prints commands.  To run,
  ## pipe to `/bin/sh`, `xargs -n1 -P$(nproc)`..  E.g.:
